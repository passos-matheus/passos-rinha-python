...
async def process_batch(batch, worker_id):
    ...
    if health_status:
        ...
        _is_default_failing: bool = status["default"]["failing"]
        _is_fallback_failing: bool = status["fallback"]["failing"]

        is_all_processors_failing = _is_default_failing and _is_fallback_failing
        if is_all_processors_failing:
